...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — then closes out by running this skill set's own **`code-review`** — **`/mattpocock-skills:code-review`** on a plugin install, **`/code-review`** otherwise; Claude Code's built-in `/code-review` is a different review, so don't substitute it here unless asked — a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and this skill set's **`code-review`** on its own whenever you want to review a branch or PR against a fixed point.
